...
//    cmd_parser = new command_parser_mocap();
    ...
    while (cmd_parser->fetch_next() && !robot.do_go_home()) {
        ...
        Serial.print(" } firstX=");
        Serial.print(cmd_parser->is_first_directionX() ? "true" : "false");
        Serial.print(", time constraint=");
        ...
        while ((cur_cmd = pl->get_next_command()) != nullptr) {
            ...
            if (robot.do_go_home()) {
                Serial.println("button interrupted the execution");
                Serial.flush();
                break;
            }
        ...
        if (!robot.do_go_home()) {
            ...
            Serial.println("route done, fetching next command...");
        }
    }

    Serial.println("Escaped the main execution loop");
    robot.stop();
